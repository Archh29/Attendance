      ...
      {attendance.length > 0 ? (
        attendance.map((record) => (
          <div key={record.yearLevel} className="bg-white shadow-md rounded-lg overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full bg-white">
                {renderTableHeader(['Name', 'October 7', 'October 8'])}
                <tbody>
                  {record.students.length > 0 ? (
                    record.students.map((student, index) => (
                      <tr key={index} className="border-t border-gray-200 hover:bg-gray-50">
                        <td className="py-3 px-4 text-sm text-gray-700">{student.name}</td>
                        <td className="py-3 px-4 text-sm text-gray-700">{student.dates['2024-10-07'] || 'No record'}</td>
                        <td className="py-3 px-4 text-sm text-gray-700">{student.dates['2024-10-08'] || 'No record'}</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      ...
      <div className="bg-white shadow-2xl rounded-lg p-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
          ...
          <div>
            <label htmlFor="reportType" className="block text-gray-700 mb-2">Report Type</label>
            <select
              id="reportType"
              value={reportType}
